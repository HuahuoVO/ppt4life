...
def process_image(path, fit=None, max_width=0, quality=88):
    """按 fit 居中裁剪到目标比例/尺寸并缩放（cover）。返回 (w,h)；未处理返回 None。"""
    if not fit and not max_width:
        return None
    try:
        from PIL import Image
    except Exception:
        print("! 未安装 Pillow，跳过裁剪/压缩（原图保留）。装：pip install Pillow", file=sys.stderr)
        return None
    ...
    try:
        ...
        for i, u in enumerate(picked, 1):
            ...
            try:
                ...
                dims = process_image(dest, fit, args.max_width)
                if dims:
