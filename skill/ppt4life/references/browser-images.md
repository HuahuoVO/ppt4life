...
技能内附 `scripts/fetch-images.py`，把上面第 1–3 节的流程封装成一条命令（自动起会话 → 搜图 → 抽链 → 下载 → 停会话，并写来源清单）：
...
# 换来源 / 中文关键词
python scripts/fetch-images.py --query "科研 实验室" --source pexels --count 4

# 直接裁成可用尺寸：居中裁剪到 16:9 + 限宽 1920（需 Pillow）
python scripts/fetch-images.py --query "product launch" --fit 16:9 --max-width 1920

# 统一观感：白底圆角 + 细描边，让来源各异的图看起来一致（需 Pillow）
python scripts/fetch-images.py --query "product launch" --style card

# 额外生成一页 16:9“图片来源.pptx”（便于答辩/汇报标注出处；需 python-pptx）
python scripts/fetch-images.py --query "product launch" --cite

# 只看计划不执行
python scripts/fetch-images.py --query "product launch" --dry-run
```

要点：
- `--source` 支持 `unsplash` / `pexels` / `pixabay`（免版权）与 `bing` / `baidu`（搜索引擎）。
- 下载的图片落在 `--out`（默认 `./assets/img`），并自动生成 `_sources.json` 与 **`_sources.md`**（来源清单）。
- **`--fit` / `--max-width`**（需 Pillow）：`--fit 16:9`（或 `4:3`/`1:1`/`1920x1080`）会**居中裁剪**到目标比例，`--max-width 1920` 等比限宽——把大图直接变成版式可用的尺寸（实测 6000×4000/2.2MB → 1920×1080/304KB）。未装 Pillow 时自动跳过、保留原图。
- **`--style card`**（需 Pillow）：给每张图套**白底圆角 + 细描边**，让来自不同图库、明暗各异的素材**观感统一**（避免拼在一起显杂乱）。可与 `--fit` 叠加使用。
- 加 `--cite` 会额外生成一张 **`图片来源.pptx`**（16:9，一页表格：文件 ↔ 来源 URL + 版权提示），可直接附在 PPT 末尾；未装 `python-pptx` 时会自动跳过并提示。
- 脚本已在**单进程内**串行执行所有 `bsk` 调用（因 bsk 的 session 不跨 shell 存活），并对每个调用设超时，卡住会**优雅中止**而不是假死。
- 找不到 `bsk` 时会给出安装提示并退出码 2，不会静默失败。
- 拿到图片后，按 `visuals.md` 第 5 节做**清晰度/比例/风格**检查再上版。
